...
extension GameScene {
    ...
    func addPlayer() {
        player = SKSpriteNode(color: UIColor.black, size: kPLAYER_SIZE)
    ...
    func addBoundries() {
        ...
        leftEdge.physicsBody = SKPhysicsBody(edgeFrom: CGPoint(x: 0,y: -kEDGE_EXTENSION_LENGTH), to: CGPoint(x:0, y: self.frame.height + kEDGE_EXTENSION_LENGTH))
        leftEdge.name = "BOUNDARY"
        rightEdge.physicsBody = SKPhysicsBody(edgeFrom: CGPoint(x: self.frame.width,y: -kEDGE_EXTENSION_LENGTH), to: CGPoint(x:self.frame.width, y: self.frame.height + kEDGE_EXTENSION_LENGTH))
        rightEdge.name = "BOUNDARY"
        
        mover.physicsBody = SKPhysicsBody(edgeFrom: CGPoint(x: 0, y: self.frame.height / 2), to: CGPoint(x: self.frame.width, y: self.frame.height / 2 ))
        mover.name = "MOVER"
    ...
    func addLevel(_ height: CGFloat) {
        var colorOfLevel = UIColor.black
        numLevels += 1
